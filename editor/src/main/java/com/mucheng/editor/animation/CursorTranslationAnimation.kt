...
open class CursorTranslationAnimation(protected val editor: MuCodeEditor) : CursorAnimation,
    ...
    private val animatorLineHighlightBottomY = ValueAnimator()

    private var startX = 0f

    private var startY = 0f

    private var lineHighlightY = 0f
    ...
    override fun markAnimationBegin(line: Int, row: Int) {
        ...
        lineHighlightY = (painters.getLineHeight() * line).toFloat()
    }
    ...
    override fun markAnimationFinish(line: Int, row: Int) {
        ...
        if (isRunning()) {
            startX = animatedX()
            startY = animatedY()
            lineHighlightY = animatedHighlightLineBottomY()
            cancel()
        }
        val layout = editor.layout
        val painters = editor.styleManager.painters
        ...
        val endLineHighlightEndY = (painters.getLineHeight() * line).toFloat()

        animatorX.setFloatValues(startX, endX)
        animatorY.setFloatValues(startY, endY)
        animatorLineHighlightBottomY.setFloatValues(lineHighlightY, endLineHighlightEndY)

        animatorX.duration = duration
        animatorY.duration = duration
        animatorLineHighlightBottomY.duration = duration
        // 若多次进行动画说明需要瞬间执行
        if (System.currentTimeMillis() - lastAnimatedTime < 100) {
            animatorX.duration = 0
            animatorY.duration = 0
            animatorLineHighlightBottomY.duration = 0
        }
    }
    ...
    override fun start() {
        ...
        animatorX.start()
        animatorY.start()
        animatorLineHighlightBottomY.start()
    ...
    override fun cancel() {
        animatorX.cancel()
        animatorY.cancel()
        animatorLineHighlightBottomY.cancel()
    }
    ...
    override fun isRunning(): Boolean {
        return animatorX.isRunning || animatorY.isRunning || animatorLineHighlightBottomY.isRunning
    }
    ...
    override fun animatedHighlightLineBottomY(): Float {
        return animatorLineHighlightBottomY.animatedValue as Float
    }
