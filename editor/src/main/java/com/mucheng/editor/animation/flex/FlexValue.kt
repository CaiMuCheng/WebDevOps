...
class FlexValue(editor: MuCodeEditor, initialVal: Float, speed: Float) : FlexObject(editor, speed) {
    ...
    override fun update() {
        if (isEqualApproximately(currentVal, targetVal)) return
        currentVal = lerp(currentVal, targetVal)
        currentVal = round(currentVal, targetVal)
